...
function getFriendlyIntro(rows) {
  ...
  if (rows.length > 1) {
    return "Absolutely. Let's make this simple:";
  }

  if (category.includes("admission")) {
    return "Of course. Here's the simple answer:";
  }

  if (category.includes("tuition") || category.includes("fees")) {
    return "Sure. Here's the fee breakdown:";
  }

  if (category.includes("course") || category.includes("computer science")) {
    return "Sure. Here's a neat course breakdown:";
  }

  if (category.includes("contact")) {
    return "Of course. Here are the contact details:";
  }
